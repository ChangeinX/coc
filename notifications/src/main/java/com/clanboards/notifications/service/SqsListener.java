      ...
      for (var msg : messages) {
        ...
        try {
          ...
          try {
            ...
            userId = userRepository.findBySub(rawUser).map(User::getId).orElse(null);
          }
        ...
        if (ok) {
          ...
          int receiveCount =
              Integer.parseInt(msg.attributes().getOrDefault("ApproximateReceiveCount", "1"));
          if (receiveCount >= 5) {
            if (dlqUrl != null && !dlqUrl.isEmpty()) {
              sqsClient.sendMessage(
                  SendMessageRequest.builder().queueUrl(dlqUrl).messageBody(msg.body()).build());
            }
            sqsClient.deleteMessage(
                DeleteMessageRequest.builder()
                    .queueUrl(queueUrl)
