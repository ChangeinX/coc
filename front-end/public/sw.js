...
const COOKIES_PATH = '/cookies-20250729.html';
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches
      .open(CACHE_NAME)
      .then((cache) => cache.addAll([COOKIES_PATH, PRIVACY_PATH, TERMS_PATH]))
      .catch(() => {})
  );
});
...
self.addEventListener('fetch', (event) => {
  ...
  if (
    ...
    (url.pathname === '/privacy' || url.pathname === '/privacy.html')
  ) {
    event.respondWith(cacheFirst(COOKIES_PATH));
    return;
  }
  if (url.origin === self.location.origin && url.pathname === COOKIES_PATH) {
    event.respondWith(cacheFirst(COOKIES_PATH));
    return;
  }
  if (
    url.origin === self.location.origin &&
    (url.pathname === '/privacy-policy' || url.pathname === '/privacy-policy.html')
  ) {
  ...
  if (
    url.origin === self.location.origin &&
    (url.pathname === '/terms' || url.pathname === '/terms.html')
  ) {
